...
/**
 * \file    fichemag/class/actions_fichemag.class.php
 * \ingroup fichemag
...
/**
 * Class ActionsFicheMag
 */
class ActionsFicheMag extends CommonHookActions
{
	/**
	...
	public function addMoreMassActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		/* print_r($parameters); print_r($object); echo "action: " . $action; */
		if (in_array($parameters['currentcontext'], array('somecontext1', 'somecontext2'))) {		// do something only for the context 'somecontext1' or 'somecontext2'
			$this->resprints = '<option value="0"'.($disabled ? ' disabled="disabled"' : '').'>'.$langs->trans("FicheMagMassAction").'</option>';
		}
	...
	public function loadDataForCustomReports($parameters, &$action, $hookmanager)
	{
		...
		if ($parameters['tabfamily'] == 'fichemag') {
			$head[$h][0] = dol_buildpath('/module/index.php', 1);
			$head[$h][1] = $langs->trans("Home");
			$head[$h][2] = 'home';
			$h++;

			$this->results['title'] = $langs->trans("FicheMag");
			$this->results['picto'] = 'fichemag@fichemag';
		}
		...
		$arrayoftypes = array();
		//$arrayoftypes['fichemag_myobject'] = array('label' => 'MyObject', 'picto'=>'myobject@fichemag', 'ObjectClassName' => 'MyObject', 'enabled' => isModEnabled('fichemag'), 'ClassPath' => "/fichemag/class/myobject.class.php", 'langs'=>'fichemag@fichemag')
	...
	public function restrictedArea($parameters, $object, &$action, $hookmanager)
	{
		...
		if ($parameters['features'] == 'myobject') {
			if ($user->hasRight('fichemag', 'myobject', 'read')) {
	...
	public function completeTabsHead(&$parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($parameters['mode'] == 'remove') {
			...
			if (in_array($element, ['context1', 'context2'])) {
				...
				$parameters['head'][$counter][0] = dol_buildpath('/fichemag/fichemag_tab.php', 1) . '?id=' . $id . '&amp;module='.$element;
				$parameters['head'][$counter][1] = $langs->trans('FicheMagTab');
				if ($datacount > 0) {
					$parameters['head'][$counter][1] .= '<span class="badge marginleftonlyshort">' . $datacount . '</span>';
				}
				$parameters['head'][$counter][2] = 'fichemagemails';
				$counter++;
			}
	...
	public function showLinkToObjectBlock($parameters, &$object, &$action, $hookmanager)
	{
		...
		$this->results = array('myobject@fichemag' => array(
			'enabled' => isModEnabled('fichemag'),
			'perms' => 1,
